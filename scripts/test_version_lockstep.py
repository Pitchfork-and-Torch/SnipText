...
def main() -> int:
    ...
    assert f"v{__version__} Read Image" in readme, "README current package drift"

    html = _read("landing/public/index.html")
    assert f"SnipText {__version__} Read Image" in html, "landing title drift"
    assert f'"softwareVersion": "{__version__}"' in html, "JSON-LD softwareVersion drift"
    assert f"{__version__} Read Image · Pitchfork-and-Torch" in html, "eyebrow drift"
    ...
    assert f"Version: {__version__} Read Image" in llms, "llms.txt version drift"
    root_llms = _read("llms.txt")
    assert f"Version: {__version__} Read Image" in root_llms, "repo-root llms.txt version drift"

    sitemap = _read("landing/public/sitemap.xml")
    assert f"SnipText {__version__} Read Image" in sitemap, "sitemap title drift"
    ...
    assert example.get("clipboard_hotkey") == "ctrl+shift+i", "config.example missing clipboard_hotkey"
    assert example.get("output_format") == "markdown", "config.example output_format drift"
