...
def main() -> int:
    ...
    assert f"v{__version__} The Ledger" in readme, "README current package drift"

    html = _read("landing/public/index.html")
    assert f"SnipText {__version__} The Ledger" in html, "landing title drift"
    assert f'"softwareVersion": "{__version__}"' in html, "JSON-LD softwareVersion drift"
    assert f"{__version__} The Ledger · Pitchfork-and-Torch" in html, "eyebrow drift"
    ...
    assert f"Version: {__version__} The Ledger" in llms, "llms.txt version drift"
    root_llms = _read("llms.txt")
    assert f"Version: {__version__} The Ledger" in root_llms, "repo-root llms.txt version drift"

    sitemap = _read("landing/public/sitemap.xml")
    assert f"SnipText {__version__} The Ledger" in sitemap, "sitemap title drift"
