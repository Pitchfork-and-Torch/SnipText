...
def main() -> None:
    ...
    missing = [p.name for p in (src_logo, src_hero, src_steps) if not p.is_file()]
    if missing:
        raise SystemExit("Missing brand source files: " + ", ".join(missing))
    ...
    draw.text((220, 178), "2.0.0 Read Image  -  Snip. Read. Clipboard.", font=font(22), fill=(91, 219, 255, 255))
    draw.text((72, 280), "Click. Drag. Or a copied image.", font=font(42, True), fill=(241, 245, 249, 255))
    draw.text((72, 340), "Text from a region, a replay, or a picture", font=font(24), fill=(203, 213, 225, 255))
    draw.text((72, 376), "already on the clipboard. The picture stays in memory.", font=font(24), fill=(203, 213, 225, 255))
    ...
    cards_info = [
        ...
        (1080, "Paste", "Text lands on your\nclipboard. Or read an\nimage you already copied."),
    ]
