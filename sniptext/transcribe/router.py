...
class EngineRouter:
    ...
    def output_format(self) -> str:
        fmt = str(self.config.get("output_format") or "markdown").strip().lower()
        if fmt not in ("markdown", "plain", "line"):
            return "markdown"
        return fmt

    def _apply_format(self, result: TranscriptResult) -> TranscriptResult:
        if not result.ok:
            return result
        fmt = self.output_format()
        text = shape_output(result.text, fmt)
    ...
    def transcribe(self, image: Image.Image) -> TranscriptResult:
        ...
        prefer_md = self.output_format() == "markdown"

        if mode == "local_only":
            return self._apply_format(self._run_local(image, languages, prefer_md))

        if mode == "ai_only":
            result = self._run_vision(image, languages, prefer_md)
            if result.ok:
                return self._apply_format(result)
            return TranscriptResult(
                text="",
