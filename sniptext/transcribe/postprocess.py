...
def to_one_line(text: str) -> str:
    """Collapse whitespace so a snip can paste as a single clipboard line."""
    return " ".join((text or "").split())


def shape_output(text: str, output_format: str) -> str:
    """markdown keeps the engine text. plain strips markdown. line is one line of plain text."""
    fmt = (output_format or "markdown").strip().lower()
    if fmt == "plain":
        return maybe_plain(text, False)
    if fmt == "line":
        return to_one_line(maybe_plain(text, False))
    return text or ""
