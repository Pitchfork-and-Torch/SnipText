...
def region_to_store(
    ...
    *,
    ...
    """Return a region only after the grab succeeded. A failed grab keeps the previous file."""
    if not captured:
        return None
    return rect_from_region(region_from_rect(rect))


def save_last_region(rect: tuple[int, int, int, int]) -> None:
    """Replace last_region.json via a temp file so a crash cannot leave a half write."""
    path = last_region_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    payload = json.dumps(region_from_rect(rect), indent=2) + "\n"
