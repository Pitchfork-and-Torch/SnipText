...
class HotkeyService:
    ...
    def start_many(
        ...
        """One listener for every shortcut. Duplicate combos are not registered twice."""
        self.stop()
        mapping, dropped = assign_hotkeys(bindings)
        if not mapping:
            return False, [spec for spec, _ in bindings]
        ...
        try:
            ...
            listener.start()
            self._listener = listener
            self._spec = ",".join(mapping.keys())
            return True, dropped
        except Exception as exc:
            print(f"[SnipText] hotkey start failed: {exc}", file=sys.stderr)
            self._listener = None
            return False, [spec for spec, _ in bindings]
...
def assign_hotkeys(
    bindings: list[tuple[str, Callable[[], None]]],
) -> tuple[dict[str, Callable[[], None]], list[str]]:
    """Map pynput combos to callbacks.

    A later spec that normalizes to an earlier one is dropped. A spec with no
    modifier is dropped. This is the self-collision guard (snip vs replay vs
    clipboard image) so two listeners never register the same combo.
    """
    mapping: dict[str, Callable[[], None]] = {}
    seen: set[str] = set()
    dropped: list[str] = []
    for spec, callback in bindings:
        norm = normalize_hotkey(spec or "")
        ...
        if norm in seen:
            ...
            continue
        seen.add(norm)
        mapping[hotkey_to_pynput(norm)] = callback
