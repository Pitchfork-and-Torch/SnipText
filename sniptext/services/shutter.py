...
def play_shutter(enabled: bool = True, player=None) -> bool:
    """Fire-and-forget. Never raise into the snip pipeline.

    Pass ``player`` to record the path without playing audio (tests).
    Returns True only when a sound would be started.
    """
    if not enabled:
        return False
    path = shutter_path()
