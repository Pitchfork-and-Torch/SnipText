...
class SnipTextApp:
    def __init__(self) -> None:
        ...
        self.replay_hotkeys = HotkeyService()
    ...
    def _start_replay_hotkey(self) -> None:
        spec = self.config.get("replay_hotkey") or "ctrl+shift+r"
        ok = self.replay_hotkeys.start(spec, self._on_replay_hotkey)
        if ok:
            log.info("Replay hotkey registered: %s", spec)
        else:
            log.warning("Replay hotkey failed; use tray Replay last snip")
