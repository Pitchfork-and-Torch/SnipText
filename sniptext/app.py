...
from sniptext.services.image_in import load_image_file
...
class SnipTextApp:
    ...
    def _hotkey_bindings(self) -> list[tuple[str, object]]:
        return [
            (str(self.config.get("hotkey") or "ctrl+shift+t"), self._on_hotkey),
            (str(self.config.get("replay_hotkey") or "ctrl+shift+r"), self._on_replay_hotkey),
            (str(self.config.get("clipboard_hotkey") or "ctrl+shift+i"), self._on_clipboard_hotkey),
        ]

    def _start_hotkeys(self) -> None:
        ok, dropped = self.hotkeys.start_many(self._hotkey_bindings())
        if ok and dropped:
            log.warning("Hotkey not registered (duplicate or invalid): %s", ", ".join(dropped))
            notify(
                __app_name__,
                "A shortcut matches another one, so only the first is active. Change it in Settings.",
                enabled=True,
            )
        if ok:
            log.info("Hotkeys registered: %s", self.hotkeys.current)
        else:
            log.warning("Hotkey failed; use tray menu New snip")
            notify(
                __app_name__,
                "Hotkey failed. Use the tray menu.",
                enabled=True,
            )

    def _on_replay_hotkey(self) -> None:
        try:
            self.root.after(0, self.replay_last_snip)
        except Exception:
            self.replay_last_snip()

    def _on_clipboard_hotkey(self) -> None:
        try:
            self.root.after(0, self.read_clipboard_image)
        except Exception:
    ...
    def replay_last_snip(self) -> None:
        """Re-capture the last selected region without drawing a new overlay."""
        rect = load_last_region()
        if rect is None:
            notify(__app_name__, "No last snip region yet. Do a New snip first.", True)
            return
        if not self._try_acquire():
            return
        try:
            log.info("Replay last region %s", rect)
            image = capture_region(rect)
        except Exception as exc:
            self._release_snip()
            log.exception("Replay capture failed")
            notify(__app_name__, f"Replay failed: {exc}", self.config.get("notifications", True))
            return
        self._remember_region(rect)
    ...
    def read_clipboard_image(self) -> None:
        """Transcribe an image that is already on the clipboard. Does not grab the screen."""
        if not self._try_acquire():
            return
        try:
            ...
            self._release_snip()
            log.exception("Clipboard image read failed")
            notify(__app_name__, f"Could not read clipboard image: {exc}", True)
            return
        if image is None:
            self._release_snip()
            notify(
                __app_name__,
                "No image on the clipboard. Copy a picture, or use New snip.",
                True,
            )
            return
        log.info("Clipboard image %sx%s", image.width, image.height)
        self._last_image = image
        try:
            self._transcribe_async(image)
        finally:
    ...
    def read_image_file(self) -> None:
        """Transcribe a local image file. The file is read into memory and not copied into app data."""
        ...
        if not self._try_acquire():
            return
        try:
            image = load_image_file(Path(path))
        except Exception as exc:
            self._release_snip()
            log.exception("Image file read failed")
            notify(__app_name__, str(exc) or "Could not read that image", True)
            return
        log.info("File image %sx%s", image.width, image.height)
        self._last_image = image
        try:
            self._transcribe_async(image)
        finally:
    ...
    def _remember_region(self, rect: tuple[int, int, int, int]) -> None:
        stored = region_to_store(rect, captured=True)
        if stored is None:
            return
        try:
            save_last_region(stored)
        except OSError as exc:
            log.warning("Could not save last region: %s", exc)
    ...
    def _build_menu(self):
        ...
        def about(icon=None, item=None) -> None:
            def _a() -> None:
                ...
                messagebox.showinfo(
                    ...
                    f"Snip: {self.config.get('hotkey')}\n"
                    f"Replay: {self.config.get('replay_hotkey')}\n"
                    f"Clipboard image: {self.config.get('clipboard_hotkey')}\n"
        ...
        def read_clip(icon=None, item=None) -> None:
            self.root.after(0, self.read_clipboard_image)

        def read_file(icon=None, item=None) -> None:
            self.root.after(0, self.read_image_file)

        return pystray.Menu(
            Item("New snip", snip, default=True),
            Item("New snip (3s delay)", snip_delay),
            Item("Replay last snip", lambda icon=None, item=None: self.root.after(0, self.replay_last_snip)),
            Item("Read clipboard image", read_clip),
            Item("Read image file...", read_file),
