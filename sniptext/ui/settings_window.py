...
from sniptext.services.hotkey import assign_hotkeys, normalize_hotkey
...
class SettingsWindow:
    ...
    def _build_prefs_tab(self, tab: tk.Frame) -> None:
        ...
        self._label(pad, "Replay last region", bold=True).pack(anchor="w", padx=12, pady=(14, 2))
        self._label(
            pad,
            "Recaptures the last successful snip. Saved only after the grab works.",
            muted=True,
        ).pack(anchor="w", padx=12)
        self.vars["replay_hotkey"] = tk.StringVar(
            value=str(self.config.get("replay_hotkey") or "ctrl+shift+r")
        )
        self._hotkey_entry(pad, self.vars["replay_hotkey"])

        self._label(pad, "Read clipboard image", bold=True).pack(anchor="w", padx=12, pady=(14, 2))
        self._label(
            pad,
            "Reads a picture already on the clipboard. Does not replace copied text when no image is there.",
            muted=True,
        ).pack(anchor="w", padx=12)
        self.vars["clipboard_hotkey"] = tk.StringVar(
            value=str(self.config.get("clipboard_hotkey") or "ctrl+shift+i")
        )
        self._hotkey_entry(pad, self.vars["clipboard_hotkey"])
        ...
        ttk.Combobox(
            ...
            values=["markdown", "plain", "line"],
            state="readonly",
        ).pack(fill="x", padx=12)
        self._label(
            pad,
            "line copies one line of plain text. markdown and plain keep line breaks.",
            muted=True,
    ...
    def _hotkey_entry(self, parent: tk.Misc, variable: tk.StringVar) -> None:
    ...
    def _save(self) -> None:
        ...
        cfg["replay_hotkey"] = normalize_hotkey(str(self.vars["replay_hotkey"].get()))
        cfg["clipboard_hotkey"] = normalize_hotkey(str(self.vars["clipboard_hotkey"].get()))
        ...
        if self.on_save:
            self.on_save(cfg)
        _, dropped = assign_hotkeys(
            [
                (cfg["hotkey"], lambda: None),
                (cfg["replay_hotkey"], lambda: None),
                (cfg["clipboard_hotkey"], lambda: None),
            ]
        )
        note = "Settings saved."
        if dropped:
            note += (
                "\n\nA shortcut matches another one, so only the first stays active: "
                + ", ".join(dropped)
            )
        messagebox.showinfo("SnipText", note, parent=self.win)
        try:
            self.win.destroy()
        except tk.TclError:
            pass
