...
[![Version](https://img.shields.io/badge/version-2.0.0-5BDBFF)](https://github.com/Pitchfork-and-Torch/SnipText/releases/latest)
...
- Tray app: Clip Desk (search, pin, copy, export/import a local JSON ledger), Replay last snip (`Ctrl+Shift+R`), Read clipboard image (`Ctrl+Shift+I`) or an image file, Connect AI, delay snip, settings
- Output as markdown, plain text, or one line
- Snip pictures stay in memory. Clip Desk stores text only. The last region is saved only after a grab succeeds.
...
**Current package: v2.0.0 Read Image** (region snip, replay last region, clipboard image, or an image file)
...
4. Tray icon + short "running" banner - hotkey `Ctrl+Shift+T`, clipboard image `Ctrl+Shift+I`, or tray **New snip**
